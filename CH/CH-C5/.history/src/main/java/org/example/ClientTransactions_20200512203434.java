...
public class ClientTransactions
{
    ...
    public ClientTransactions () {
        try {
            ...
            millis+=60000;
            // 1 minuto de vida la transacción

           
           
            MsgStructure transaction = new MsgStructure("david", "nico", 100, millis);
            Gson convert = new Gson();
            this.queueChannel.basicPublish("", this.queueName, MessageProperties.PERSISTENT_TEXT_PLAIN, tra.getBytes());
            /*
            while (true) {
                s = ss.accept();
